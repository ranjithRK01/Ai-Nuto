...
const {
  ...
  getCurrentPlan,
  setActivePlan,
} = require('../utils/mongoVectorDB');
const {
  generateRAGResponse,
  generatePlanSummary,
} = require('../utils/aiResponse');
  ...
  try {
    if (!req.file) {
      return res.status(400).json({
        ...
        message:
          'Please upload a PDF, DOCX, or text file containing your meal plan.',
      });
    }
    ...
    if (chunks.length === 0) {
      ...
      return res.status(400).json({
        ...
        message:
          "The document appears to be empty or doesn't contain meal plan information.",
        suggestions: [
...
const clearPlan = async (req, res) => {
  try {
    ...
    try {
      ...
      console.warn(
        'Warning: Could not deactivate plan:',
