  ...
  try {
    ...
    try {
      ...
      return res.status(400).json({
        ...
        details:
          'The PDF file may be corrupted, password-protected, or in an unsupported format. Please try with a different PDF file or convert it to a different format.',
    ...
    if (!pdfData.text || pdfData.text.trim().length === 0) {
      ...
      return res.status(400).json({
        ...
        details:
          'The PDF appears to be empty or contains only images. Please upload a PDF with text content.',
        suggestions: [
          'Ensure the PDF contains selectable text (not just images)',
...
const askQuestion = async (req, res) => {
  try {
    ...
    if (topChunks.length === 0) {
      return res
        .status(404)
        .json({ error: 'No relevant information found for this question' });
    }
      ...
      messages: [
        {
          role: 'system',
          content:
            'You are a helpful nutrition assistant. Answer questions based only on the provided nutrition plan information.',
        },
        {
          role: 'user',
          content: prompt,
