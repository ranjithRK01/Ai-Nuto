...
// Connect to MongoDB
mongoose.connect(
  'mongodb+srv://ranjithRk01:IH7Z2BW2rBJTTTYu@reqsta.ntqrp.mongodb.net/reqsta?retryWrites=true&w=majority&appName=nutrition-ai',
  {
...
async function viewVectorDatabase() {
  try {
    ...
    if (totalChunks === 0) {
      console.log(
        '❌ No data found. Please upload a PDF first using POST /api/upload'
      );
      return;
    }
    ...
    chunks.forEach((chunk, index) => {
      ...
      console.log(
        `  Embedding Sample: [${chunk.embedding.slice(0, 5).join(', ')}...]`
      );
      console.log('');
    });
    ...
    if (chunks.length > 0) {
      ...
      console.log(
        `  Database Size: ~${((totalChunks * embeddingLength * 8) / 1024).toFixed(2)} KB`
      );
      console.log(
        `  Average Text Length: ${Math.round(chunks.reduce((sum, chunk) => sum + chunk.text.length, 0) / chunks.length)} characters`
      );
    }
  } catch (error) {
