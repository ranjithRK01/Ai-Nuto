...
async function testBillGeneration() {
  ...
  try {
    ...
    console.log('✅ Menu items:', menuResponse.data.menuItems.length, 'items found');
    ...
    for (let i = 0; i < testVoiceInputs.length; i++) {
      ...
      try {
        ...
        bill.items.forEach(item => {
          console.log(`      - ${item.quantity}x ${item.itemName}: ₹${item.totalPrice}`);
        });
        console.log('');
        ...
        console.log('❌ Error generating bill:', error.response?.data?.message || error.message);
        console.log('');
      }
    }
    ...
    if (billsResponse.data.bills.length > 0) {
      ...
      console.log(`   Created: ${new Date(firstBill.createdAt).toLocaleString()}`);
    }
...
async function testMenuManagement() {
  ...
  try {
    ...
    const updateResponse = await axios.put(`${BASE_URL}/menu/${addResponse.data.menuItem._id}`, {
      price: 200,
      description: 'Updated description'
    });
    console.log('✅ Item updated:', updateResponse.data.menuItem.name, 'Price: ₹', updateResponse.data.menuItem.price);
    console.log('');
    ...
    console.error('❌ Menu management test failed:', error.response?.data?.message || error.message);
  }
}
